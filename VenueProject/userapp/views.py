...
def bookdata(request, id):
     if 'id' in request.session:
          ...
          if Booking.objects.filter(date_time=date_time,venueid=Venuedb.objects.get(id=id)).exists():
            ...
            data = Booking(venueid=Venuedb.objects.get(id=id),userid=Register.objects.get(id=userid),date_time=date_time,purpose=purpose,selected_service=services,service_name=service_name,total=totalcost)
            data.save()
