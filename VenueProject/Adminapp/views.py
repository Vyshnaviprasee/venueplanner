...
def getdata(request):
    if request.method=="POST":
        ...
        price=price,phone=phone,email=email,services=services,venue_img=venue_img)
        data.save()
...
def editupdate(request,id):
    if request.method == 'POST':
        ...
        services_c=request.POST.getlist('service')
        ...
        try:
            ...
            Venuedb.objects.filter(id=id).update(venue_name=name_c,address=address_c,manager=man_c,description=vdesc_c,
                price=price_c,phone=phone_c,email=email_c,services=services_c,venue_img=file)
    return redirect('viewVenue')   
